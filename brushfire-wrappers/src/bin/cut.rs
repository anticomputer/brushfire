use brushfire_wrappers::{
    ...
    print_policy_error, schemas::CUT_SCHEMA,
};
use std::path::Path;
...
fn main() {
    ...
    let file_args = parse_args(&CUT_SCHEMA, &args);

    for file_arg in file_args {
        let path = Path::new(&file_arg.path);
        if let Err(e) = policy.check_file_access(path, file_arg.access_mode) {
            print_policy_error(CUT_SCHEMA.name, path, &e);
            std::process::exit(1);
        }
    }

    exec_real_utility(&CUT_SCHEMA, &args);
}
