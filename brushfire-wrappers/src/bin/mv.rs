...
fn main() {
    ...
    let file_args = parse_args(&MV_SCHEMA, &args);
    ...
    for file_arg in file_args {
        ...
        if let Err(e) = policy.check_file_access(path, file_arg.access_mode) {
            print_policy_error(MV_SCHEMA.name, path, &e);
            std::process::exit(1);
        }
    }

    // All checks passed - exec real utility
    exec_real_utility(&MV_SCHEMA, &args);
}
