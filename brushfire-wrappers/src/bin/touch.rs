//! Wrapper for `touch` utility with policy enforcement.

use brushfire_wrappers::{exec_real_utility, get_policy_engine, parse_args, print_policy_error};
use brushfire_wrappers::schemas::TOUCH_SCHEMA;
use std::path::Path;
...
fn main() {
    ...
    let file_args = parse_args(&TOUCH_SCHEMA, &args);
    ...
    for file_arg in file_args {
        ...
        if let Err(e) = policy.check_file_access(path, file_arg.access_mode) {
            print_policy_error(TOUCH_SCHEMA.name, path, &e);
            std::process::exit(1);
        }
    }

    // All checks passed - exec real utility
    exec_real_utility(&TOUCH_SCHEMA, &args);
}
